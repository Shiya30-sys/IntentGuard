...
AI agents can perform multiple inference and tool calls during an interaction.

However, not every user message requires another AI-agent call.
...
These messages do not require another AI-agent call.

Sending unnecessary messages through an agent can result in additional inference, token usage, latency, API usage, and compute.
...
If the message requires no action, IntentGuard stops it.

If the message is a genuine task, IntentGuard allows it to continue to the AI system.
...
This demonstrates why a context-aware gate can be more useful than judging every message in isolation.
...
The purpose of IntentGuard is not to claim that it eliminates AI energy consumption.

Instead, it explores whether unnecessary AI-agent calls can be prevented before inference occurs.

Reducing unnecessary calls can potentially reduce:
...
Direct energy savings require further measurement and are part of the future work for this project.
...
- The current implementation does not directly measure electricity or energy savings.
